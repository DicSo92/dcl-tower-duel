...
export class LeaderBoard {
    ...
    constructor() {
        ...
        // this.layout.setParent(this.global)
        // this.layout.addComponent(
        //     new Transform({
        //         position: new Vector3(0, 0, 0),
        //         rotation: Quaternion.Euler(0, 180, 0),
        //         scale: new Vector3(6, 5, 1)
        //     })
        // )
        // this.layout.addComponent(new PlaneShape())
        this.updateBoard()
    ...
    async buildLeaderBoard(scoreData: any[], parent: Entity, length: number) {
        ...
        if (this.scoreBoardNames.length === 0) {
            this.nameTitle = new ScoreBoardText(TextTypes.BIGTITLE, 'Player', { position: new Vector3(-0.8, 0.65, 0) }, parent)
            this.scoreTitle = new ScoreBoardText(TextTypes.BIGTITLE, 'Score', { position: new Vector3(0.8, 0.65, 0) }, parent)

            for (let i = 0; i < length; i++) {
                if (i < scoreData.length) {
                    const name = new ScoreBoardText(TextTypes.TINYTITLE, scoreData[i].name, { position: new Vector3(-0.6, 0.2 - i / 4, 0) }, parent)
                    this.scoreBoardNames.push(name)

                    const score = new ScoreBoardText(TextTypes.TINYVALUE, scoreData[i].score.toString(), { position: new Vector3(0.6, 0.2 - i / 4, 0) }, parent)
...
export class ScoreBoardText extends Entity {
    TiltleFont = new Font(Fonts.SansSerif_Heavy)
    SFFont = new Font(Fonts.SansSerif)
    constructor(
        type: TextTypes,
        text: string,
